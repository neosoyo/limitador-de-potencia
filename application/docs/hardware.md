...
*   Data Format (CSV):
    ...
    *Example Output:*
    ```csv
    34200,120.5,10.2,11.8,450.2,1500,1420,1420
    ```
...
| State | Condition | LED Color | Blink Frequency | Blink Period |
|---|---|---|---|---|
| **ERROR** | $PWM_{in} < 900\,\mu\text{s}$ or $PWM_{in} > 2000\,\mu\text{s}$ | **Red** (e.g., `#FF0000`) | **2 Hz** | Blinks every 500ms (250ms ON, 250ms OFF) |
| **OK** | System ready and healthy, $PWM_{out} = PWM_{in}$ | **Green** (e.g., `#00FF00`) | **1 Hz** | Blinks every 1s (500ms ON, 500ms OFF) |
| **LIMITING**| ADRC actively limiting power, $PWM_{out} < PWM_{in}$ | **Orange** (e.g., `#FF4500`) | **4 Hz** | Blinks every 250ms (125ms ON, 125ms OFF) |

---

## 2. Non-Volatile Storage (NVS) Configuration

To ensure parameter persistence across power cycles, all device configuration variables are stored in the internal flash memory using Zephyr's Non-Volatile Storage (NVS) file system.
...
### NVS Operational Rules
*   **Mounting NVS:** On startup, the NVS file system mounts the storage partition (typically `storage_partition`).
*   **Loading Configuration:**
    *   The system attempts to read the `device_config` struct from NVS ID `1`.
    *   If no config is found (first-time boot), a set of sensible default values is written to NVS, and the device proceeds with these defaults.
*   **Writing Configuration:** Any update to the ADRC gains, target power, team info, or PIN via the shell triggers an immediate write back to NVS to ensure durability.
...
### BLE Link Encryption & PIN-Passkey Security

For operator privacy and security, all telemetry characteristics require **Encrypted Link Pairing with Man-in-the-Middle (MITM) protection**.
*   **Passkey Pairing:** The 6-digit PIN code configured in the NVS `PIN_code` structure acts as the Bluetooth pairing password (Passkey).
...
*   **Split Binary Integer Stream:** 18 bytes total per packet $\times$ 10Hz = **180 bytes/second** (1440 bps).
*   **Net Bandwidth Saving:** **~67% reduction** in radio airtime, lowering power consumption and eliminating frame fragmentation. Static identification data (Team Name and Number) are read once on connection, avoiding unnecessary periodic bandwidth consumption.
